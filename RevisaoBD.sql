...
GO
INSERT INTO aluno(ra,nome,idade)
...
GO
INSERT INTO titulacao(codigo,titulo)
VALUES  (1,'Especialista'),
		(2,'Mestre'),
		(3,'Doutor')
GO
INSERT INTO professor(registro,nome,titulacao_codigo)
VALUES  (1111,'Leandro',2),
		...
		(1121,'Julio',1)
GO
INSERT INTO curso(codigo,nome,area)
...
VALUES  (1,3416),
		...
		(8,3499)
GO
INSERT INTO disciplina_professor(disciplina_codigo,professor_registro)
VALUES  (1,1111),
        (2,1112),
	    (3,1113),
	    (4,1114),
...
WHERE d.codigo = cd.disciplina_codigo
...
SELECT d.nome AS disciplina,
       p.titulacao_codigo AS titulacao
FROM disciplina d, professor p, disciplina_professor dp
WHERE d.codigo = dp.disciplina_codigo
  AND p.registro =  dp.professor_registro
ORDER BY d.nome
...
SELECT DISTINCT d.nome AS disciplina,
       COUNT(ad.aluno_ra) as qtd_alunos
...
SELECT CASE WHEN (COUNT(ad.aluno_ra) >= 5) 
	   THEN
	      d.nome
	   END AS disciplina,  
	   CASE WHEN (COUNT(ad.aluno_ra) >= 5) 
	   THEN
	      p.nome
	   END AS professor
FROM disciplina d, professor p, aluno_disciplina ad, disciplina_professor dp
WHERE d.codigo = dp.disciplina_codigo
  AND p.registro = dp.professor_registro
...
SELECT c.nome, 
	   COUNT(DISTINCT(p.registro)) AS quantidade
FROM disciplina d, professor p, curso c, disciplina_professor dp, curso_disciplina cd
